...
async def search_documents(query: str, k: int = 3) -> str:
    """
    대한민국헌법과 관련된 질문에 대해 유사한 문서를 검색하는 도구입니다.
    Args:
        query: 검색할 질문이나 키워드 (예: "대통령 임기", "국회의원 선거")
        k: 검색할 문서 개수 (기본값: 3)
    Returns:
        str: 검색된 헌법 문서 내용과 메타데이터
    """
    ...
    documents = [(doc, distance) for (doc, distance) in results if distance < 0.4]

    # LLM으로 보낼 내용을 리스트 항목으로 만들기
    output_lines = [ f"검색 결과: '{query}' (대한민국헌법)\n" ]
    ...
    def __init__(self, model:str="openai:gpt-4o-mini") -> None:
        ...
        self.agent = create_agent(
            ...
            system_prompt="""
                ...
                사용 가능한 도구:
                **search_documents**: 헌법 문서 검색
                
                답변 생성 지침:
                - 사용자의 질문을 받으면 search_documents 도구를 사용하여 관련 헌법 조항을 검색합니다.
                - 검색된 헌법 문서 내용을 바탕으로 정확하고 상세한 답변을 제공하세요.
                - 답변 시 조항 번호, 페이지 등 출처를 반드시 명시하여 신뢰성을 높이세요.
                - 헌법에 없는 내용은 추측하지 말고 "헌법 문서에서 해당 정보를 찾을 수 없습니다"라고 답하세요.
            """
        )
